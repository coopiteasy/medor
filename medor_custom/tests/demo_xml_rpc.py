...
def demo_xml_rpc():
    ...
    stats = models.execute_kw(
        ...
        "read",
        [1],
        {"fields": ["nb_subscribers", "nb_cooperators"]},
    )
    print(stats)

    deposit_points = models.execute_kw(
        DB, uid, PASSWORD, "res.company", "get_deposit_point", [], {}
    )
    print(deposit_points)

    subscription = models.execute_kw(
        DB, uid, PASSWORD, "res.users", "get_subscription", [1], {}
    )
    print(subscription)

    subscription = models.execute_kw(
        DB, uid, PASSWORD, "res.users", "get_subscription", [5910], {}
    )
    print(subscription)

    subscription = models.execute_kw(
        DB, uid, PASSWORD, "res.users", "get_subscription", [2881], {}
    )
    print(subscription)
